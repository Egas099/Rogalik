...
{ // Система атаки. Основной метод Fire() вызывается, когда юнит "хочет" выстрелить
    ...
    {    // Выстреливает из position в сторону direction
    ...
    public void Fire(Vector3 position, Transform parent)
    { // Создаёт объект в position 
        if ((last_fire_range + (6 / entity.attack_speed)) < Time.time)
        {
            last_fire_range = Time.time;
    ...
    public void Fire(Vector3 position, Vector2 direction, string type)
    {    // Выстреливает из position в сторону direction
        switch (type)
        {
            case "single":
    ...
    { // Вызывается каждый кадр при столкновении с чем-нибудь
